...
def mock_get_stac_items(mocker):
    ...
    mock_func = mocker.patch(
        "dps_stac_item_generator.handler.get_stac_documents", return_value=[mock_item]
    )
...
def test_handler_deduplicates_hierarchy_documents_per_invocation(
    mock_context, mock_sns_client, mock_get_stac_items
):
    """Repeated records publish hierarchy documents once but retain each Item."""
    s3_event_data = {
        "bucket": {"name": "test-catalog-bucket"},
        "object": {"key": "path/catalog.json"},
    }
    event = create_sqs_event_with_s3_notification([s3_event_data, s3_event_data])
    catalog = {
        "type": "Catalog",
        "stac_version": "1.1.0",
        "id": "user-dXNlcg",
    ...
    collection = {
        ...
        "stac_version": "1.1.0",
        "id": "user__algo__1",
        "title": "algo 1 outputs",
        "description": "DPS outputs generated by algo (1) for user.",
        "license": "proprietary",
        "extent": {
    ...
    mock_get_stac_items.return_value = [
        catalog,
        collection,
        mock_get_stac_items.mock_item,
    ]

    assert item_gen_handler.handler(event, mock_context) is None
    assert mock_sns_client.publish.call_count == 4
    published = [
        json.loads(call.kwargs["Message"])
...
def test_handler_partial_hierarchy_publication_is_retryable(
    mock_context, mock_sns_client, mock_get_stac_items
):
    """A failed hierarchy publish fails the SQS record after prior SNS work."""
    s3_event_data = {
        "bucket": {"name": "test-catalog-bucket"},
        "object": {"key": "path/catalog.json"},
    }
    event = create_sqs_event_with_s3_notification([s3_event_data])
    mock_get_stac_items.return_value = [
        {"type": "Catalog", "stac_version": "1.1.0", "id": "user-a"},
        {"type": "Collection", "stac_version": "1.1.0", "id": "collection"},
        mock_get_stac_items.mock_item,
    ]
    mock_sns_client.publish.side_effect = [
        {"MessageId": "catalog-published"},
        Exception("SNS publish failed"),
    ]

    result = item_gen_handler.handler(event, mock_context)

    assert result == {"batchItemFailures": [{"itemIdentifier": "sqs-msg-id-0"}]}
    assert mock_sns_client.publish.call_count == 2
